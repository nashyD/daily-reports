 ...
 * The SCRIPT is written by the scout agent (an LLM already) — this script only
 * does voices + encoding + metadata, so there's no separate script-gen cost.
 *
 * Usage:
 *   node scripts/make-podcast.mjs --date 2026-06-06 --script /tmp/ep.txt \
 *        --title "TSD Daily — Jun 6" --summary "Today's AI, tech, and US headlines."
 *
 *   # pipeline test with no API call (4s of silence):
 *   node scripts/make-podcast.mjs --mock --date 2026-06-06 --title "Test" --summary "Test"
 *
 * Env:
 *   GEMINI_API_KEY     (required unless --mock)
 *   PODCAST_TTS_MODEL  default gemini-2.5-flash-preview-tts
 *   PODCAST_HOST_A/B   speaker labels in the script   (default Alex / Sam)
 *   PODCAST_VOICE_A/B  Gemini prebuilt voices          (default Puck / Kore)
 *   PODCAST_KEEP       how many recent episodes to retain (default 60)
 *
 * The script file must label every line with the host, e.g.:
 *   Alex: Morning. Big one out of Anthropic today...
 *   Sam: Right, the Series H — let's get into it.
 *
...
const KEEP       = Number(process.env.PODCAST_KEEP || 60);
const TTS_MODEL  = process.env.PODCAST_TTS_MODEL || 'gemini-2.5-flash-preview-tts';
const HOST_A     = process.env.PODCAST_HOST_A || 'Alex';
const HOST_B     = process.env.PODCAST_HOST_B || 'Sam';
const VOICE_A    = process.env.PODCAST_VOICE_A || 'Puck';
const VOICE_B    = process.env.PODCAST_VOICE_B || 'Kore';
...
const date = arg('date');
const title = arg('title') || `TSD Daily — ${date}`;
...
const tmp = join(tmpdir(), `tsd-pod-${date}-${process.pid}`);
